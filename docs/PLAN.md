...
4. Планировщик атомарно забирает до `ccr.report.worker-concurrency` заданий со статусом `pending`, переводит их в
...
3. Использовать ограниченный пул обработчиков; число worker-ов задаётся через `ccr.report.worker-concurrency`.
