...
3. API проверяет соответствие `report_type` и ветки `source_query`, сохраняет `report_job(status = pending)` и возвращает `report_id`.
...
7. Внутри этой транзакции обработчик потоково читает актуальное состояние через серверный курсор, порциями.
8. CSV записывается во временный артефакт:
    - локальный временный файл или
    - временный ключ объекта в хранилище, который не публикуется во внешнем API
9. После полной записи файл хешируется, загружается в итоговый ключ объекта, затем создается `report_file`.
10. Только после успешной публикации файла задание завершается в статусе `created`.
11. Если возникает ошибка, обработчик удаляет временный файл или объект и не создает `report_file`.
12. При временной ошибке задание возвращается в `pending` с новым `next_attempt_at`.
13. Отдельный процесс контроля таймаутов переводит зависшие задания в `timed_out`.
14. Клиентская часть получает статусы через `GetReports` и `GetReport`.
15. Для скачивания клиентская часть вызывает `GeneratePresignedUrl(file_id)` и получает ссылку, для которой TTL принудительно ограничивается на стороне сервиса.
...
3. Во вкладке `Reports` отображается таблица:
    ...
    - `rows_count`
    - `actions`
4. Для `period` интерфейс использует `source_query.time_range`, который возвращается в `Report`.
...
8. Для `failed` и `timed_out` доступна `Retry` через повторный `CreateReport` с тем же `source_query`.
...
1. Источник и обязательность полей `shop_name`, `wallet_name`, `provider_name`, `terminal_name` пока окончательно не зафиксированы. На текущем этапе их нужно рассматривать как возможные поля для денормализованного поиска, а не как гарантированную часть `v1`.
